...
const validateAlphabeticalWithSpaces = (value: string) => /^[\p{L} ]+$/u.test(value);
    ...
    joinDate: Yup.date()
        ...
        .test(
            ...
            function (value) {
                ...
                return new Date(value) >= new Date(new Date(dateOfBirth).setFullYear(new Date(dateOfBirth).getFullYear() + 18));
            }
        )
        .test(
            'is-not-weekend',
